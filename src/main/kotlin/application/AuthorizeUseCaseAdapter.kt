...
class AuthorizeUseCaseAdapter
{
    fun toAuthorizeUseCase(payment: Payment): AuthorizeUseCaseResponse =
        ...
        when (payment)
        {
            ...
            is ReadyForAuthenticationAndAuthorizeContinuation -> AuthorizeUseCaseResponse.InvalidPaymentStatus(payment.payload.id)
            is ReadyForAuthenticationClientAction -> AuthorizeUseCaseResponse.InvalidPaymentStatus(payment.payload.id)
            is ReadyForAuthenticationContinuation -> AuthorizeUseCaseResponse.InvalidPaymentStatus(payment.payload.id)
